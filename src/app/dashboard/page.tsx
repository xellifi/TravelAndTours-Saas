...
export default async function DashboardOverview() {
  ...
  return (
    <div>
      <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-5 sm:mb-8">Dashboard Overview</h1>

      {!business ? (
        <div className="bg-primary-50 border border-primary-200 p-6 sm:p-8 rounded-2xl sm:rounded-3xl text-center">
            <h2 className="text-lg sm:text-xl font-bold text-primary-900 mb-3 sm:mb-4">Complete your Profile</h2>
            <p className="text-primary-700 text-sm sm:text-base mb-5 sm:mb-6">You haven&apos;t created a business landing page yet. Let&apos;s get started!</p>
            <a href="/dashboard/settings" className="btn-primary px-7 py-3 rounded-full text-white font-bold inline-block text-sm sm:text-base">Create Business</a>
        </div>
      ) : (
        <>
...
const StatCard = ({ icon, label, value, color, bgColor }: { icon: string; label: string; value: number; color: string; bgColor: string }) => (
  <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-50 flex items-center gap-4 sm:gap-5">
    <div className={`w-12 h-12 sm:w-14 sm:h-14 ${bgColor} ${color} rounded-2xl flex items-center justify-center text-lg sm:text-xl flex-shrink-0`}>
      <i className={`fas ${icon}`}></i>
    </div>
    <div className="min-w-0">
      <p className="text-[11px] sm:text-xs font-bold text-gray-500 uppercase tracking-wider leading-tight">{label}</p>
