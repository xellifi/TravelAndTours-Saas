...
import { adminUpdateBusinessAction, type BusinessFormState } from './actions';
...
export default function EditBusinessDialog({
  ...
  // Owner options: current owner + everyone who owns nothing.
  const ownerOptions = useMemo<UserOption[]>(() => {
    const list: UserOption[] = [...eligibleUsers];
    if (!list.find((u) => u.id === business.owner_id)) {
      list.unshift({
  ...
  }, [eligibleUsers, business.owner_id, ownerEmail]);
                ...
                >
                  {ownerOptions.map((u) => (
                    <option key={u.id} value={u.id}>
                      ...
                      {u.id === business.owner_id ? ' (current)' : ''}
                    </option>
                  ))}
                </select>
                <p className="text-xs text-gray-500 mt-2">
                  Only users without a business can be assigned a new one.
                </p>
              </div>
